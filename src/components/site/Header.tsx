  ...
  return (
    ...
    >
      <div className="container-narrow flex items-center justify-between h-20">
        ...
        <nav className="hidden sm:flex flex-wrap items-center justify-end gap-x-6 gap-y-2">
          {navItems.map((item) => (
            <NavLink
              key={item.label}
              to={item.to}
      ...
      {/* Mobile overlay */}
      <div
        className={`sm:hidden fixed inset-0 top-20 bg-background transition-opacity duration-300 ${
